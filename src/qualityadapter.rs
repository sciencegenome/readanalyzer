use crate::filestruct::{FastqWindow1, FastqWindow2, FileFastqPre};
use std::error::Error;
use std::fs::File;
...
pub fn r1adapter_process(
    ...
    let mut fastq_window_1: Vec<FastqWindow1> = Vec::new();

    for i in 0..writecleaned_header_1.len() {
        fastq_window_1.push(FastqWindow1 {
            header: writecleaned_header_1[i].clone(),
            sequence: writecleaned_sequence_1[i].clone(),
            strand: writecleaned_strand_1[i].clone(),
            quality: writecleaned_quality_1[i].clone(),
            start: writecleaned_sequence_1[i].find(adapter).unwrap(),
            end: writecleaned_sequence_1[i].find(adapter).unwrap() + adapter.len(),
        })
    }

    let mut finalversion_r1: Vec<FileFastqPre> = Vec::new();
    for i in fastq_window_1.iter() {
        finalversion_r1.push(FileFastqPre {
            header: i.header.clone(),
            sequence: i.sequence[i.start..i.end].to_string(),
            strand: i.strand.clone(),
            quality: i.quality.clone(),
        })
    }

    let mut fileopen_1 = File::create("all-drop1.fastq").expect("file not present");
    for i in finalversion_r1.iter() {
        write!(
            fileopen_1,
            "{}\n{}\n{}\n{}\n",
...
pub fn r2adapter_process(
    ...
    let mut fastq_window_2: Vec<FastqWindow2> = Vec::new();

    for i in 0..writecleaned_header_2.len() {
        fastq_window_2.push(FastqWindow2 {
            header: writecleaned_header_2[i].clone(),
            sequence: writecleaned_sequence_2[i].clone(),
            strand: writecleaned_strand_2[i].clone(),
            quality: writecleaned_quality_2[i].clone(),
            start: writecleaned_sequence_2[i].find(adapter).unwrap(),
            end: writecleaned_sequence_2[i].find(adapter).unwrap() + adapter.len(),
        })
    }

    let mut finalversion_r2: Vec<FileFastqPre> = Vec::new();
    for i in fastq_window_2.iter() {
        finalversion_r2.push(FileFastqPre {
            header: i.header.clone(),
            sequence: i.sequence[i.start..i.end].to_string(),
            strand: i.strand.clone(),
            quality: i.quality.clone(),
    ...
    let mut fileopen_2 = File::create("all-drop2.fastq").expect("file not present");
    for i in finalversion_r2.iter() {
        write!(
            fileopen_2,
            "{}\n{}\n{}\n{}\n",
            i.header, i.sequence, i.strand, i.quality,
